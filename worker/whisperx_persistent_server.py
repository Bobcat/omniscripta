...
from whisperx_import_runner import _apply_torch_thread_tuning, _as_positive_int, _cleanup_torch
...
class PersistentWhisperxRunner:
  ...
  def transcribe(self, envelope: dict[str, Any]) -> dict[str, Any]:
    ...
    if diarize_enabled:
      return {
        ...
        "resolved_options": resolved,
        "error": {
          "code": "ASR_UNSUPPORTED_OPTION",
          "message": "persistent live_chunk runner does not support diarize_enabled=true",
          "retryable": False,
          "details": {"option": "diarize_enabled"},
        },
        "warnings": [],
      }
...
def _handle_command(runner: PersistentWhisperxRunner, cmd_obj: dict[str, Any]) -> bool:
  ...
  try:
    ...
    response = runner.transcribe(envelope)
