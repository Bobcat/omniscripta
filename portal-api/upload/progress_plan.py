...
def _topics_tail_seconds(record: dict[str, Any], *, topics_enabled: bool) -> float | None:
  phases = record.get("phase_seconds") or {}
  if not isinstance(phases, dict):
    return None
  post = _safe_float(phases.get(TOPICS_PREP_PHASE))
  if post is None:
    return None
  total = max(0.0, float(post))
  if topics_enabled:
    llm = _safe_float(phases.get("llm_topics"))
    if llm is None:
      return None
    total += max(0.0, float(llm))
  else:
    llm = _safe_float(phases.get("llm_topics_skipped"))
    total += max(0.0, float(llm or 0.0))
  return total
...
def build_prediction(
  *,
  ...
  for phase in phase_order:
    if phase in {"llm_topics", "llm_topics_skipped"}:
      vals: list[float] = []
      for r in candidates:
        total = _topics_tail_seconds(r, topics_enabled=bool(phase == "llm_topics"))
