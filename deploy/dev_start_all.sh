...
# Start the full dev stack in a stable order and wait for key HTTP endpoints.
# Useful because ASR pool warm startup can take noticeable time.
...
systemctl --user start "$BATCH_WORKER_UNIT" "$LIVE_WORKER_UNIT" "$LLM_WORKER_UNIT"
